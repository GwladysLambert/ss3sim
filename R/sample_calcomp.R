...
sample_calcomp <- function(dat_list, outfile, fleets = c(1,2), years,
  ...
  if (fixed.number==F) {
    ...
    for(i in 1:length(fleets)){
      ...
      lencomp.fl <- lencomp[lencomp$FltSvy == fl &
                              lencomp$Yr %in% years[[i]], ]
      ...
      for(yr in years[[i]]) {
        ...
        # define newcomp for length so ESS can be changed later
        newcomp.len <- lencomp.fl[lencomp.fl$Yr==yr, ]
  ...
  if (fixed.number==T) {
    ...
    for(i in 1:length(fleets)){
      ...
      for(yr in years[[i]]) {
        ...
        #           } else {
        #             ## case of overdispersed:
        MAX.Nsamp.ages.per.lbin <- rmultinom(n=1, size=Nsamp.len, prob=prob.len)
        #           }
        #           if(any(is.na(Nsamp.ages.per.lbin)))
        #             stop("Invalid age sample size for a length bin in calcomp")
        ## This is where the actual sampling takes place
        ...
        newcomp$Nsamp <- MAX.Nsamp.ages.per.lbin # max possible number of samples limited by number of lengths
        
        for(ll in 1:nrow(newcomp)){
          newcomp$Nsamp[ll] <- ifelse(Nsamp[[i]][yr.ind] <newcomp$Nsamp[ll],Nsamp[[i]][yr.ind],newcomp$Nsamp[ll])
          N.temp <- newcomp$Nsamp[ll]
          if(N.temp>0){
            cal.temp <-
              rmultinom(n=1, size=N.temp,#Nsamp.ages.per.lbin[ll]
                        prob=as.numeric(newcomp[ll,-(1:9)]))
          } else {
            cal.temp <- -1
          }
